export type Surah = { id: number; name: string; ayahs: number };
...
].map(([id, name, ayahs]) => ({ id: id as number, name: name as string, ayahs: ayahs as number }));

export const getSurah = (id: number | string) => SURAH_LIST.find((surah) => surah.id === Number(id)) ?? SURAH_LIST[0];
export const arabicNumber = (value: number | string) => new Intl.NumberFormat("ar-SA").format(Number(value));

/** Standard 30-juz (part) boundaries: [juzNumber, startSurah, startAyah]. */
const JUZ_BOUNDARIES: Array<[number, number, number]> = [
...
/** Compares (surah, ayah) reading position. Positive if a is further than b. */
function comparePosition(aSurah: number, aAyah: number, bSurah: number, bAyah: number) {
  if (aSurah !== bSurah) return aSurah - bSurah;
  return aAyah - bAyah;
}

/**
 * Number of juz' fully completed given the furthest (surah, ayah) reading position reached.
 * A juz counts as complete once the reader has passed its end boundary (the start of the next juz).
 */
export function completedJuzCount(furthestSurah: number, furthestAyah: number) {
  let completed = 0;
  for (let i = 0; i < JUZ_BOUNDARIES.length; i += 1) {
    const next = JUZ_BOUNDARIES[i + 1];
    if (!next) break;
    const [, nextSurah, nextAyah] = next;
    if (comparePosition(furthestSurah, furthestAyah, nextSurah, nextAyah) >= 0) completed = i + 1;
  }
  return completed;
}
